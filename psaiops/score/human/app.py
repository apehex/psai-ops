...
TITLE = '''Human Scoring'''
INTRO = list((bool(__i) * ' ' + __t, _LEGEND.get(__t, '50')) for (__i, __t) in enumerate('''Leverages an open source LLM as critic to take apart the text sections written by a human from those generated by an AI .'''.split(' ')))
TUTO = open(os.path.join(_PATH, 'tuto.md'), 'r').read()
DOCS = open(os.path.join(_PATH, 'docs.md'), 'r').read()
...
def create_metrics_block(label: str='Selection', prefix: str='') -> dict:
    __metrics = gradio.CheckboxGroup(label=label, type='value', value=[RAMPING, UNICODE, SURPRISAL, PERPLEXITY], choices=[('Ramping', RAMPING), ('Unicode', UNICODE), ('Surprisal', SURPRISAL), ('Perplexity', PERPLEXITY), ('Intermediate', INTERMEDIATE)], interactive=True)
